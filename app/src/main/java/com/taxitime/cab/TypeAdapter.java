...
import com.taxitime.cab.Objects.TypeObject;
...
public class TypeAdapter  extends RecyclerView.Adapter<TypeAdapter.viewHolders> {
    ...
    public TypeAdapter.viewHolders onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View layoutView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_marker, null, false);
    ...
    public void onBindViewHolder(final @NonNull viewHolders holder, int position) {
        ...
        if(selectedItem.equals(itemArrayList.get(position))){
            holder.mLayout.setBackgroundColor(context.getResources().getColor(R.color.colorAccent));
        }else{
    ...
    static class viewHolders extends RecyclerView.ViewHolder {
        ...
        viewHolders(View itemView) {
            super(itemView);
            mImage = itemView.findViewById(R.id.profile_image_driver);
            mPeople = itemView.findViewById(R.id.profile_image);
            mName = itemView.findViewById(R.id.name);
            mLayout = itemView.findViewById(R.id.rel1);
        }
    }
}